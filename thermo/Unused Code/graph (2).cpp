...
Graph::Graph(bool isWeighted) :
    ...
    vertexCounter(0), weighted(isWeighted) { /* nothing */ }
...
{
    assertExists(u, __FUNCTION__);
    assertExists(v, __FUNCTION__);
    assertConnected(u, v, __FUNCTION__);
...
Vertex Graph::insertVertex(string label /* = "" */)
{
    ...
    pair<size_t, bool> check = graph.insert(make_pair(vertex, EdgeMap()));
    vertexLabels.insert(make_pair(vertex, label));

    if (check.second == false)
    {
	    int new_weight = getEdgeWeight(vertex, last_inserted) + 1;
	    setEdgeWeight(vertex, last_inserted, new_weight);
    }
